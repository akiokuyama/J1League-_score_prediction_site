...
- `outputs/prediction_history/*.json` と `data/processed/matches_2026_special_clean.csv` を照合して `past_prediction_results.json` を生成するスクリプトを追加しました。
...
現時点では、`outputs/prediction_history/*.json` と `data/processed/matches_2026_special_clean.csv` の `match_id` は一致しています。ただし、該当試合は `status=unplayed` かつ `home_score` / `away_score` が未確定だったため、過去予測評価の対象外でした。
