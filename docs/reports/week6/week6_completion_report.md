...
- Linux上の大文字小文字区別により `Data/` と `data/` の参照差異が出たため、この時点ではActions内で `data -> Data` のシンボリックリンクを作成するようにしました。Week7の追加対応で `Data/` に統一し、この依存は解消済みです。
...
- `Data/` と `data/` のディレクトリ名統一は、Week7の追加対応で `Data/` に寄せて解消済みです。
